...
| Planar walker が膝丈の水プールでその場足踏み | `scripts/walker_marching_in_water.py` | `outputs/walker_marching_in_water.{mp4,csv}` |
...
make march-walker-in-water  # walker が膝丈の水プールに脚を浸してその場足踏み
```
...
水中足踏み版では同じ変数に加えて `WATER_LEVEL` (剛体床 z=0 から測った水面高さ、
単位 m。デフォルト 0.5 m ≈ 膝の高さ) を上書きできます:

```bash
make march-walker-in-water WATER_LEVEL=0.4         # 浅め (脛の中ほど)
make march-walker-in-water GAIT_HZ=1.5 WATER_LEVEL=0.5
make march-walker-in-water DURATION=5.0            # 録画を長めに (デフォルト 2.4 s)
```
...
│   └── walker_marching_in_water.py # 同じ制御で下腿を水に浸して足踏み
├── assets/
...
- [x] walker が膝丈の水プールでその場足踏み (統合シーン、`make march-walker-in-water`) — ベースラインで `|x| ≤ 2.2 cm`、`|pitch| ≤ 1.1°` を達成 (dry-floor と同等)
- [ ] walker が砂でその場足踏み
- [ ] 剛体床での前進歩行、その後砂・水へ転移
